...
//查询功能
function search(){
	if(str_is_null($('#username').val()) && str_is_null($('#truename').val()) && str_is_null($('#departmentId').val()) && str_is_null($('#state').val())) {
		alert('至少有一个查询条件不为空');
		return ;
	}
	var paraStr = "&username="+$('#username').val()+"&truename="+$('#truename').val()+"&departmentId="+$('#departmentId').val()+"&state="+$('#state').val();
