...
<div class="wraper">
    ...
    <!--content部分-->
    <div class="center">
        <img class="show" src="images/home/images_15_professional/banner.jpg" />
        <div class="center_left">
            <ul>
                <li class="li_1"><a>技术专业</a></li>
                <?php foreach ($dictItem as $index => $val){?>
                    <li><a class="red" href="#"></a><a class="li_2" href="javascript:openaddr('<?=$val->dictItemCode?>','<?=$val->dictItemName?>')">>><?=$val->dictItemName?></br></a></li>
                <?}?>
            </ul>
        </div>
        <div class="center_right">
            <div class="right_head"> <a>技术专业</a>><a><?=$dictname?></a></div>
            <div class="center_list">
                <ul>
                    <?php foreach ($articles as $index => $val){?>
                        <li><a style="float:left;width: 500px;" href="javascript:openadd('<?=$val->id?>','<?=$dictname?>');void(0);"><?=$val->title?></a>日期：<?=$val->publish_date?></li>
                    <?}?>
                </ul>
            </div>
            <?php foreach ($articles as $index => $val){?>
