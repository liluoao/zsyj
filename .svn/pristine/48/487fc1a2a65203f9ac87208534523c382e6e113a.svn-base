...
<html lang="<?= Yii::$app->language ?>" xmlns="http://www.w3.org/1999/html">
    <head>
        ...
        <script type="text/javascript">
            ...
            var editUrl = "<?=yii::$app->urlManager->createUrl('inform/edit')?>";
            var delUrl = "<?=yii::$app->urlManager->createUrl('inform/delete')?>"
            var delallUrl = "<?=yii::$app->urlManager->createUrl('inform/deleteall')?>"
            var detailUrl = "<?=yii::$app->urlManager->createUrl('inform/detail')?>"
        </script>
        <script language="javascript" type="text/javascript" src="js/page.js" charset="utf-8"></script>
        <script language="javascript" type="text/javascript" src="js/admin/inform/listall.js" charset="utf-8"></script>
        ...
        <div class="pad-lr-10">
            <form name="myform" action="" method="post" id="myform" target="iframeId">
                <div class="table-list">
                    <table width="100%" cellspacing="0" id="inform_list">
                        <thead id="inform_list_head">
                        <tr align="left">
                            <th width="50px"><input type="checkbox" id='check_box' onclick="selectall('id')"/>全选</th><th width="30px" align="center">序号</th><th width="250px" align="center">通知标题</th><th width="150px" align="center">通知类型</th><th width="159px" align="center">发布时间</th><th width="150px" align="center">发送人</th><th align="center">操作</th>
                        </tr>
                        </thead>
                        <tbody id="inform_list_body">
                        <?if(!is_null($informs)){?>
                            <?$ind = 0;?>
                            <?foreach ($informs as $index => $val){?>
                                <?$ind++;?>
                            <tr align="left">
                                ...
                                <td align="center"><?=$val['informType']?></td>
                                <td align="center"><?=$val['senderDateTime']?></td>
                                <td align="center"><?=$val['sender']?></td>
                                <td align="center">
                                    <a href="javascript:openedit('<?=$val['id']?>','<?=$val['title']?>')">修改</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:deleteInfo('<?=$val['id']?>')">删除</a>
                                </td>
                           </tr>
                            <?}?>
                        <?}?>
                    ...
                    <div class="btn">
                        ...
                        <input type="button" class="buttondel" name="dosubmit" value="删除" onclick="if (confirm('您确定要删除这些通知吗？')) delopt();"/>
                    </div>
                    <div id="pages">
...
<form action="<?=yii::$app->urlManager->createUrl('inform/listall')?>" method="get" id="pageForm">
    <input type="hidden" id="page" name="page" value="<?=$pages->page?>"/>
    <input type="hidden"  name="r" value="inform/listall"/>
    <input type="hidden" id="pre-page" name="pre-page" value="<?=$pages->pageSize?>"/>
    <input type="hidden" id="title" name="title" value="<?=$para['title']?>"/>
    <input type="hidden" id="articleType" name="articleType" value="<?=$para['informType']?>"/>
