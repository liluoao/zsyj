...
<html lang="<?= Yii::$app->language ?>" xmlns="http://www.w3.org/1999/html">
    <head>
        ...
        <script type="text/javascript">
            ...
            var rdeleteUrl = "<?=yii::$app->urlManager->createUrl('role/rdelete')?>"
            var rdeleteallUrl = "<?=yii::$app->urlManager->createUrl('role/rdeleteall')?>"
            var detailUrl = "<?=yii::$app->urlManager->createUrl('user/detail')?>"
            var roleaddlistallUrl= "<?=yii::$app->urlManager->createUrl('role/roleaddlistall')?>";
            var roleaddUrl= "<?=yii::$app->urlManager->createUrl('role/roleadd')?>";
        </script>
        <script language="javascript" type="text/javascript" src="js/admin/role/roleadd.js" charset="utf-8"></script>
        <script language="javascript" type="text/javascript" src="js/page.js" charset="utf-8"></script>
    </head>
    <body>
    <div class="subnav" id="display" >
        <div class="content-menu ib-a blue line-x"> <a class="add fb" href="javascript:openaddus('<?=$roleId?>','<?=$name?>');"><em>分配用户</em></a> </div>
    </div>
        ...
        <div class="pad-lr-10">
            <div class="table-list">
                <table width="100%" cellspacing="0" id="user_list">
                    ...
                    <tr align="left">
                        <th width="80px"><input type="checkbox" id='check_box' onclick="selectall('id')"/>全选/取消</th><th width="80px">序号</th><th width="160px">用户名</th><th width="160px">真实姓名</th><th width="160px">电话号码</th><th width="80px" align="center">性别</th><th width="80px" align="center">用户状态</th><th align="center">操作</th>
                    </tr>
                    </thead>
                    ...
                    <?php foreach ($users as $index => $val){?>
                        <tr align="left">
                            ...
                            <td align="center">
                                <a href="javascript:deleteUser('<?=$val->id?>','<?=$name?>','<?=$roleId?>')">删除</a>
                            </td>
                       </tr>
                    <?}?>
                    <?}?>
                ...
                <div class="btn">
                    ...
                    <input type="button" class="buttondel" name="dosubmit" value="删除" onclick="if (confirm('您确定要删除吗？')) delopt('<?=$name?>','<?=$roleId?>');"/>
                </div>
                <div id="pages">
...
<form action="<?=yii::$app->urlManager->createUrl('role/roleadd')?>" method="get" id="pageForm">
    <input type="hidden" id="page" name="page" value="<?=$pages->page?>"/>
    <input type="hidden"  name="r" value="role/roleadd"/>
