...
TabPanel.prototype = {
  ...
  init : function(){
	...
	this.tabpanel_content.height(t_h-this.tabpanel_tab_content.get(0).offsetHeight-20);
    ...
    else if(this.scrollFinish)
    {
      ...
      tab.bind('click', function(position){
        return function(){
		  ...
		  //document.getElementById('rightMain').src=tabitem.html;
        };
      }(this.tabs.length-1));
    ...
    if(this.scrollFinish)
    {
      ...
      else
      {
        ...
        if(this.tabs[position].content.html()=='') {
		  ...
		  //document.getElementById('rightMain').src=this.tabs[position].html;
        }
        //清除掉已经打开的选项卡激活样式
