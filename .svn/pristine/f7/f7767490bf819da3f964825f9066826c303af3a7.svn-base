...
class OrderChargeController extends Controller
{
    ...
    public function actionAssign()
    {
        ...
        if(Ordercharge::find()->where('userId = :userId',[':userId'=>$userId])->all())
        {
            $ordercharges = Ordercharge::find()->where('userId = :userId',[':userId'=>$userId])->all();
            $cityids ='';
            foreach($ordercharges as $key => $data){
                $cityid = $data->areaAddress;
                $cityids .= ','.$cityid;
            }
        }else{
            $cityids ='';
    ...
    public function actionAddarea()
    {
        ...
        if(Ordercharge::find()->where('userId = :userId',[':userId'=>$userId])->all()){
            Ordercharge::deleteAll('userId = :userId',[':userId'=>$userId]);
        }
        $ids = Yii::$app->request->post('ids');
        $citys = Yii::$app->request->post('citys');
        $ids_array = explode('-',$ids);
        $citys_array = explode('-',$citys);
        foreach($ids_array as $key => $data){
            foreach($citys_array as $index => $value) {
                if($key == $index){
